        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-6 text-sm">
            ...
            <a
              ...
              className="flex items-center gap-1.5 text-[var(--text-secondary)] hover:text-[var(--foreground)] transition-colors"
            >
          ...
          <div className="flex sm:hidden items-center gap-2">
            <Link
              href="/bg-remover"
              className="px-3 py-2 text-xs font-medium text-[var(--text-secondary)] hover:text-[var(--foreground)] hover:bg-white/5 rounded-lg transition-all"
            >
              BG
            </Link>
            <Link
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2.5 text-[var(--text-secondary)] hover:text-[var(--foreground)] transition-colors"
            >
      ...
      <section className="relative pt-20 pb-16 sm:pt-28 sm:pb-24">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <p className="text-lg sm:text-xl text-[var(--text-secondary)] max-w-2xl mx-auto mb-10 leading-relaxed">
            Professional background removal, WASM-powered compression, and more. Private, fast, and
            completely free.
          </p>
          ...
          <div className="mt-12 inline-flex flex-wrap items-center justify-center gap-3 sm:gap-4 px-4 sm:px-6 py-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)]">
            {[
              ...
              <div key={s.step} className="flex items-center gap-3">
                {i > 0 && (
                  <svg
                    className="w-4 h-4 text-[var(--text-muted)] hidden sm:block"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            {TOOLS.map((tool) => {
              ...
              return (
                ...
                >
                  {isReady && (
                    <div
                      className={`absolute inset-x-0 top-0 h-px bg-gradient-to-r ${tool.gradient} opacity-0 group-hover:opacity-100 transition-opacity`}
                    />
                  )}
                  <span
                    className={`absolute top-4 right-4 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider rounded-full ${isReady ? 'bg-[var(--success-soft)] text-[var(--success)]' : 'bg-white/5 text-[var(--text-muted)]'}`}
                  >
                    {tool.badge}
                  </span>
                  <div
                    className={`w-12 h-12 rounded-xl ${tool.iconBg} flex items-center justify-center mb-4 text-[var(--text-secondary)] group-hover:text-[var(--foreground)] transition-colors`}
                  >
                    {tool.icon}
                  </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="card p-6 text-center">
                <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-[var(--accent-soft)] flex items-center justify-center text-[var(--accent)]">
                  {f.icon}
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border)]">
                    <th className="text-left px-3 sm:px-6 py-4 font-medium text-[var(--text-secondary)]">
                      Feature
                    </th>
                    <th className="px-3 sm:px-6 py-4 font-semibold text-[var(--accent)]">
                      PicEdit
                    </th>
                    <th className="px-3 sm:px-6 py-4 font-medium text-[var(--text-secondary)]">
                      remove.bg
                    </th>
                    <th className="px-3 sm:px-6 py-4 font-medium text-[var(--text-secondary)]">
                      TinyPNG
                      ...
                      {[row.picedit, row.removebg, row.tinypng, row.iloveimg].map((val, j) => (
                        <td key={j} className="px-3 sm:px-6 py-3 text-center">
                          {val === true ? (
                            ...
                            <span
                              className={`text-xs font-medium ${j === 0 ? 'text-[var(--accent)]' : 'text-[var(--text-muted)]'}`}
                            >
                              {val as string}
                            </span>
                          )}
