		...
		<div className="space-y-5">
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-xs text-muted">
							Reduce image dimensions for smaller file
						</p>
					</div>
					<button
						onClick={() =>
							updateState({ compressionEnabled: !state.compressionEnabled })
						}
					...
					<div className="space-y-3 pt-2 border-t border-border">
						...
						<div className="space-y-2">
							...
							<input
								...
								onChange={(e) =>
									updateState({ compressionScale: Number(e.target.value) / 100 })
								}
								className="w-full"
							/>
						</div>
				...
				<div className="p-3 bg-surface rounded-lg">
					...
					{compressionPercent > 0 && (
						<p className="text-xs text-green-400">
							-{compressionPercent}% from original
						</p>
					)}
				</div>
			</div>
			...
			{originalSize > 0 && (
				<div className="flex items-center gap-2 text-xs text-muted">
					...
					Original: {formatBytes(originalSize)} • {state.originalWidth} ×{' '}
