...
export const FileUploader = memo(function FileUploader({
	...
	return (
        ...
        ${
			isDragging
				? `border-accent bg-accent/10 shadow-[0_0_30px_rgba(224,122,95,0.1)]`
				: 'border-border hover:border-accent/40 bg-elevated/50'
		}
		...
		>
			...
			{children || (
				<div className="flex flex-col items-center gap-4 px-6 text-center">
					{/* Icon */}
					<div className="p-4 rounded-2xl bg-accent/10 shadow-lg shadow-accent/5 transition-transform group-hover:scale-105">
						<svg
					...
					{formats.length > 0 && (
						<div className="flex items-center gap-2 text-xs text-muted">
							{formats.map((fmt) => (
								<span
									key={fmt}
									className="px-2.5 py-1 rounded-md bg-white/5 border border-white/6 hover:border-accent/20 transition-colors"
								>
									{fmt}
								</span>
							))}
					...
					{showPasteHint && multiple && (
						<p className="sm:hidden text-xs text-muted">Multiple files supported</p>
					)}
				</div>
			)}
		</motion.div>
