  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-1 p-1 rounded-xl bg-[var(--bg-primary)]">
          {METHOD_TABS.map((tab) => (
            <button
              key={tab.id}
            ...
            >
              {config.method === tab.id && (
                <motion.div
                  layoutId="method-tab"
                  className="absolute inset-0 bg-[var(--bg-elevated)] rounded-lg border border-[var(--border)]"
          ...
          >
            <div className="flex items-end gap-2">
              ...
              <div className="flex-1 space-y-1">
                <label className="text-xs text-[var(--text-muted)]">Width (px)</label>
                <input
                  type="number"
                  value={config.width}
                  ...
                  min={1}
                  max={16384}
                  className="w-full px-3 py-2 rounded-lg bg-[var(--bg-primary)] border border-[var(--border)] text-sm text-[var(--foreground)] focus:outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]/30 disabled:opacity-50 font-mono"
                />
              </div>
              ...
              <button
                ...
                disabled={disabled}
                className={`p-2.5 rounded-lg border transition-all mb-[1px] ${
                  config.lockAspectRatio
                    ? 'bg-[var(--accent)]/10 border-[var(--accent)]/30 text-[var(--accent)]'
                    : 'bg-[var(--bg-primary)] border-[var(--border)] text-[var(--text-muted)]'
                } hover:bg-[var(--accent)]/15 disabled:opacity-50`}
                title={config.lockAspectRatio ? 'Aspect ratio locked' : 'Aspect ratio unlocked'}
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              {/* Height */}
              <div className="flex-1 space-y-1">
                <label className="text-xs text-[var(--text-muted)]">Height (px)</label>
                <input
                  type="number"
                  value={config.height}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-[var(--text-muted)]">Fit Mode</label>
                ...
                {FIT_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`px-2 py-2 rounded-lg text-xs transition-all border ${
                      config.fit === opt.id
                        ? 'bg-[var(--accent)]/10 border-[var(--accent)]/30 text-[var(--accent)]'
                        : 'bg-[var(--bg-primary)] border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--foreground)]'
                    } disabled:opacity-50`}
                  >
          ...
          >
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-mono text-[var(--accent)]">{config.percentage}%</span>
              </div>
              <input
                type="range"
                min={1}
            ...
            {presetCategories.map(([category, presets]) => (
              <div key={category}>
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 rounded-lg bg-[var(--bg-primary)] hover:bg-[var(--bg-hover)] transition-colors"
                >
                  <span className="text-xs font-medium text-[var(--foreground)]">{category}</span>
                  <svg
                    className={`w-3.5 h-3.5 text-[var(--text-muted)] transition-transform ${expandedCategory === category ? 'rotate-180' : ''}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                    ...
                    >
                      <div className="grid grid-cols-2 gap-1 pt-1 pl-1">
                        {presets.map((preset) => (
                          ...
                          >
                            <div
                              className={`text-xs font-medium ${config.presetId === preset.id ? 'text-[var(--accent)]' : 'text-[var(--foreground)]'}`}
                            >
                              {preset.label}
                            </div>
      ...
      {config.outputFormat !== 'png' && (
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <label className="text-xs text-[var(--text-muted)]">Quality</label>
